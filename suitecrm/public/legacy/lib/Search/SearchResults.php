...
class SearchResults
{
    ...
    public function getHitsAsBeans(): array
    {
        ...
        foreach ($searchHits as $module => $beans) {
            // Skip if results key doesn't exist or is empty
            if (!isset($beans['results']) || empty($beans['results'])) {
                continue;
            }
    ...
    protected function formatForDisplay(SugarBean $obj, array $fieldDefs): SugarBean
    {
        ...
        foreach ($fieldDefs as $fieldDef) {
            ...
            if (empty($fieldName) || !property_exists($obj, $fieldName)) {
                continue;
            }
            $value = $obj->{$fieldName};
            if (isset($value)) {
                switch ($fieldDef['type']){
                    ...
                    case 'currency':
                        ...
                        if (substr($fieldDef['name'], -9) !== '_usdollar') {
                            ...
                            if (!empty($params['currency_id']) && isset($locale->currencies[$params['currency_id']]['symbol'])) {
