...
class LF_WeeklyPlanViewReporting extends SugarView
{
    ...
    private function getStageProgressionData($weekStart, $repId = null)
    {
        ...
        // Load stage probabilities for movement detection
        $probabilities = LF_PRConfig::getConfigJson('stages', 'stage_probabilities');
        if (!is_array($probabilities)) {
            $probabilities = [];
        }

        // Get all reports for this week, optionally filtered by rep
        $reportSql = "SELECT wr.id AS report_id, wr.assigned_user_id
                      FROM lf_weekly_report wr
                      WHERE wr.week_start_date = " . $db->quoted($weekStart) . "
                        AND wr.deleted = 0";
        if ($repId) {
            $reportSql .= " AND wr.assigned_user_id = " . $db->quoted($repId);
        }

        $reportResult = $db->query($reportSql);
        $reportIds = [];
        while ($row = $db->fetchByAssoc($reportResult)) {
        ...
        if (!empty($reportIds)) {
            $idList = implode(',', array_map([$db, 'quoted'], $reportIds));

            // Get all snapshots for these reports
            $snapSql = "SELECT rs.opportunity_id, rs.stage_at_week_start, rs.stage_at_week_end,
                               rs.was_planned, rs.plan_category, rs.amount_at_snapshot
                        FROM lf_report_snapshots rs
                        WHERE rs.lf_weekly_report_id IN ($idList)
                          AND rs.deleted = 0";
            $snapResult = $db->query($snapSql);

            while ($snap = $db->fetchByAssoc($snapResult)) {
                $startStage = $snap['stage_at_week_start'] ?? '';
                $endStage = $snap['stage_at_week_end'] ?? '';
                $amount = (float)($snap['amount_at_snapshot'] ?? 0);

                // Detect movement
                if (in_array($endStage, ['Closed Won', 'closed_won'])) {
                    $forward++;
                } elseif (in_array($endStage, ['Closed Lost', 'closed_lost'])) {
                    $backward++;
                } elseif (!empty($startStage)) {
                    $startProb = (int)($probabilities[$startStage] ?? 0);
                    $endProb = (int)($probabilities[$endStage] ?? 0);
                ...
                // Calculate actual values for progression and new pipeline
                $startProb = (int)($probabilities[$startStage] ?? 0);
                $endProb = (int)($probabilities[$endStage] ?? 0);
                $actualProgression = $amount * ($endProb - $startProb) / 100;

                $planCategory = $snap['plan_category'] ?? '';
                if ($planCategory === 'developing') {
                    $newPipelineActual += $actualProgression;
                } elseif ($planCategory === 'progression') {
                    $progressionActual += $actualProgression;
                }
            }
        }
        ...
        return [
            ...
            'backward' => $backward,
            'static' => $static,
            'new_pipeline_actual' => $newPipelineActual,
            'progression_actual' => $progressionActual,
        ];
    }
