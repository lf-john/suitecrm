...
class LF_WeeklyPlanViewSave_json extends SugarView
{
    ...
    public function display()
    {
        ...
        } elseif (isset($input['status']) && $input['status'] === 'reviewed') {
            // Manager review - update status to reviewed with reviewer info
            $query = sprintf(
                "UPDATE lf_weekly_plan SET status = 'reviewed', reviewed_by = %s, reviewed_date = %s, date_modified = %s, modified_user_id = %s WHERE id = %s AND deleted = 0",
                $db->quoted($current_user->id),
                $db->quoted($now),
                $db->quoted($now),
                $db->quoted($current_user->id),
