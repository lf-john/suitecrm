...
class LF_WeeklyPlanViewRep_report extends SugarView
{
    ...
    public function display()
    {
        ...
        // Load live opportunity data
        $opportunities = [];
        $allOppIds = array_unique(array_merge(array_keys($planItems), array_keys($snapshots)));
        if (!empty($allOppIds)) {
        ...
        $analysisProb = (int)($probabilities[$analysisStage] ?? 1);

        foreach ($snapshots as $oppId => $snap) {
            if (isset($planItems[$oppId])) continue; // already planned
            $opp = $opportunities[$oppId] ?? null;
            if (!$opp) continue;

            $startStage = $snap['stage_at_week_start'] ?? '';
            $currentStage = $opp['sales_stage'] ?? '';
            if ($startStage === $currentStage) continue; // no change

            $startProb = (int)($probabilities[$startStage] ?? 0);
            // Categorize as developing (<= analysis) or existing (> analysis)
        ...
        foreach ($existingPipelineItems as $oppId => $item) {
            ...
            if (!$opp) continue;
            $snap = $snapshots[$oppId] ?? null;
            $this->renderPipelineRow($opp, $item, $snap, $accountNames[$oppId] ?? '', $probabilities, false);
        }
        ...
        foreach ($unplannedExisting as $oppId => $snap) {
            $opp = $opportunities[$oppId] ?? null;
            if (!$opp) continue;
            $this->renderPipelineRow($opp, null, $snap, $accountNames[$oppId] ?? '', $probabilities, true);
        }

        if (empty($existingPipelineItems) && empty($unplannedExisting)) {
            echo '<tr><td colspan="7" style="text-align: center; padding: 20px; color: #605e5c;">No existing pipeline items for this week.</td></tr>';
        }
        ...
        if (empty($developingItems) && empty($unplannedDeveloping)) {
            echo '<tr><td colspan="7" style="text-align: center; padding: 20px; color: #605e5c;">No developing pipeline items for this week.</td></tr>';
        }
        ...
        if (empty($prospectItems)) {
            echo '<tr><td colspan="7" style="text-align: center; padding: 20px; color: #605e5c;">No prospecting items planned for this week.</td></tr>';
        } else {
            foreach ($prospectItems as $item) {
                ...
                echo '<td>$' . htmlspecialchars(number_format((float)$item['expected_value'], 2)) . '</td>';
                ...
                if ($status === 'planned') {
                    ...
                    echo '<a target="_top" href="#/opportunities/record/' . htmlspecialchars($item['converted_opportunity_id']) . '">View Opportunity</a>';
                ...
                if ($status === 'planned') {
                    ...
                    echo '<label>Amount:<br><input type="number" class="conv-amount" value="' . htmlspecialchars($item['expected_value']) . '" style="width: 120px;"></label>';
     ...
     */
    private function renderPipelineRow($opp, $planItem, $snapshot, $accountName, $probabilities, $isUnplanned)
    {
        $oppId = $opp['id'];
        $currentStage = $opp['sales_stage'] ?? '';
    ...
    private function detectResult($startStage, $currentStage, $plannedStage, $probabilities, $isUnplanned)
    {
        ...
        if ($isUnplanned) {
            // Unplanned: just show direction
            if ($currentProb > $startProb) return 'progressed';
