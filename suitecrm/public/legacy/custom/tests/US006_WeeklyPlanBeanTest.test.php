<?php
/**
 * US-006: LF_WeeklyPlan Bean, Vardefs, Language, and Menu Tests
 *
 * Tests that the LF_WeeklyPlan Bean class, vardefs, language file, and Menu.php
...
assert(
    ...
    "Bean file should exist at: custom/modules/LF_WeeklyPlan/LF_WeeklyPlan.php"
);
...
// --- Happy Path: File contains class LF_WeeklyPlan extending SugarBean ---
assert(
    preg_match('/class\s+LF_WeeklyPlan\s+extends\s+SugarBean/', $beanContent) === 1,
    "Bean file must contain 'class LF_WeeklyPlan extends SugarBean'"
);
...
// --- Happy Path: $table_name = 'lf_weekly_plan' ---
assert(
    preg_match('/\$table_name\s*=\s*[\'"]lf_weekly_plan[\'"]/', $beanContent) === 1,
    "Bean must have \$table_name = 'lf_weekly_plan'"
);
echo "  [PASS] Bean has \$table_name = 'lf_weekly_plan'\n";

// --- Happy Path: $object_name = 'LF_WeeklyPlan' ---
assert(
    preg_match('/\$object_name\s*=\s*[\'"]LF_WeeklyPlan[\'"]/', $beanContent) === 1,
    "Bean must have \$object_name = 'LF_WeeklyPlan'"
);
echo "  [PASS] Bean has \$object_name = 'LF_WeeklyPlan'\n";

// --- Happy Path: $module_name = 'LF_WeeklyPlan' ---
assert(
    preg_match('/\$module_name\s*=\s*[\'"]LF_WeeklyPlan[\'"]/', $beanContent) === 1,
    "Bean must have \$module_name = 'LF_WeeklyPlan'"
);
echo "  [PASS] Bean has \$module_name = 'LF_WeeklyPlan'\n";
...
// --- Happy Path: getOrCreateForWeek references the lf_weekly_plan table ---
assert(
    str_contains($beanContent, 'lf_weekly_plan'),
    "getOrCreateForWeek() should reference the lf_weekly_plan table"
);
echo "  [PASS] getOrCreateForWeek() references lf_weekly_plan table\n";
...
// ============================================================
// Section 5: Bean getWeekEndDate() Instance Method
// ============================================================
echo "\nSection 5: Bean getWeekEndDate() Instance Method\n";

// --- Happy Path: File contains getWeekEndDate method ---
assert(
    preg_match('/public\s+function\s+getWeekEndDate\s*\(/', $beanContent) === 1,
    "Bean must have 'public function getWeekEndDate(' method"
);
echo "  [PASS] Bean has getWeekEndDate() method\n";

// --- Happy Path: getWeekEndDate has no required parameters ---
assert(
    preg_match('/function\s+getWeekEndDate\s*\(\s*\)/', $beanContent) === 1,
    "getWeekEndDate() must have no required parameters"
);
echo "  [PASS] getWeekEndDate() has no required parameters\n";

// --- Happy Path: getWeekEndDate uses DateTime for date calculation ---
assert(
    str_contains($beanContent, 'DateTime'),
    "getWeekEndDate() must use PHP DateTime for date calculation"
);
echo "  [PASS] getWeekEndDate() uses DateTime\n";

// --- Happy Path: getWeekEndDate references week_start_date property ---
assert(
    str_contains($beanContent, 'week_start_date'),
    "getWeekEndDate() must reference week_start_date property"
);
echo "  [PASS] getWeekEndDate() references week_start_date\n";

// --- Happy Path: getWeekEndDate adds 6 days (not 7) ---
assert(
    str_contains($beanContent, '6 days') || str_contains($beanContent, '+6 day'),
    "getWeekEndDate() must add 6 days to week_start_date (Mon-Sun = 6 day interval)"
);
echo "  [PASS] getWeekEndDate() adds 6 days\n";

// --- Negative Case: getWeekEndDate is NOT static (it's an instance method) ---
assert(
    preg_match('/public\s+static\s+function\s+getWeekEndDate/', $beanContent) !== 1,
    "getWeekEndDate() must NOT be static - it needs access to \$this->week_start_date"
);
echo "  [PASS] getWeekEndDate() is not static (instance method)\n";
...
assert(
    ...
    "Vardefs file should exist at: custom/modules/LF_WeeklyPlan/metadata/vardefs.php"
);
...
$tempFile = tempnam(sys_get_temp_dir(), 'us006_vardefs_');
$wrapperCode = "<?php\n";
...
// --- Happy Path: Dictionary key matches $object_name: $dictionary['LF_WeeklyPlan'] ---
assert(
    array_key_exists('LF_WeeklyPlan', $dictionary),
    "\$dictionary must have key 'LF_WeeklyPlan' matching \$object_name"
);
echo "  [PASS] \$dictionary has key 'LF_WeeklyPlan'\n";
...
$vardefEntry = $dictionary['LF_WeeklyPlan'];
...
assert(
    $vardefEntry['table'] === 'lf_weekly_plan',
    "Vardefs 'table' should be 'lf_weekly_plan', got: " . ($vardefEntry['table'] ?? 'NULL')
);
echo "  [PASS] Vardefs table is 'lf_weekly_plan'\n";
...
// --- Edge Case: submitted_date, reviewed_by, reviewed_date are NOT required ---
assert(
...
assert(
    ...
    "Language file should exist at: custom/modules/LF_WeeklyPlan/language/en_us.lang.php"
);
...
$tempFile = tempnam(sys_get_temp_dir(), 'us006_lang_');
$wrapperCode = "<?php\n";
$wrapperCode .= "define('sugarEntry', true);\n";
...
assert(
    ...
    "Menu file should exist at: custom/modules/LF_WeeklyPlan/Menu.php"
);
...
$tempFile = tempnam(sys_get_temp_dir(), 'us006_menu_');
$wrapperCode = "<?php\n";
$wrapperCode .= "define('sugarEntry', true);\n";
...
// --- Happy Path: $module_menu has at least 2 entries (planning tool + dashboard) ---
assert(
    count($moduleMenu) >= 2,
    "\$module_menu should have at least 2 entries (planning tool + dashboard), got: " . count($moduleMenu)
);
echo "  [PASS] \$module_menu has at least 2 entries\n";

// --- Happy Path: Menu contains a link to planning tool (action=planning) ---
$menuContentLower = strtolower($menuContent);
assert(
    str_contains($menuContentLower, 'action=planning'),
    "Menu.php must contain a link with action=planning for the planning tool"
);
echo "  [PASS] Menu.php has planning tool link (action=planning)\n";

// --- Happy Path: Menu contains a link to dashboard (action=dashboard) ---
assert(
    str_contains($menuContentLower, 'action=dashboard'),
    "Menu.php must contain a link with action=dashboard for the planning dashboard"
);
echo "  [PASS] Menu.php has dashboard link (action=dashboard)\n";

// --- Happy Path: Menu references the LF_WeeklyPlan module ---
assert(
    str_contains($menuContent, 'LF_WeeklyPlan'),
    "Menu.php must reference the LF_WeeklyPlan module"
);
echo "  [PASS] Menu.php references LF_WeeklyPlan module\n";
...
assert(
    preg_match('/\$object_name\s*=\s*[\'"]LF_WeeklyPlan[\'"]/', $beanContent) === 1,
    "Bean \$object_name must match vardefs dictionary key 'LF_WeeklyPlan'"
);
assert(
    array_key_exists('LF_WeeklyPlan', $dictionary),
    "Vardefs \$dictionary key must match Bean \$object_name 'LF_WeeklyPlan'"
);
...
assert(
    $vardefEntry['table'] === 'lf_weekly_plan',
    "Vardefs table must match Bean \$table_name 'lf_weekly_plan'"
);
assert(
    preg_match('/\$table_name\s*=\s*[\'"]lf_weekly_plan[\'"]/', $beanContent) === 1,
    "Bean \$table_name must match vardefs table 'lf_weekly_plan'"
);
